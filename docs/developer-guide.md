...
warning/error task log 의 `meta` 도 renderer mode 별로 다르게 보인다. line mode 는 원인 분석을 위해 `error=...`, `responseBody=...` 같은 문자열을 생략 없이 전체 출력하고, compact/Ink timeline 만 화면 제약 때문에 clip/ellipsis 를 적용한다.
